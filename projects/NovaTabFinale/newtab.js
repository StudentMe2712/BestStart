...
const FALLBACK_QUOTES = [
  { content: "В минуту нерешительности действуй быстро и старайся сделать первый шаг, хотя бы и лишний.", author: "Лев Толстой" },
  { content: "Никогда не ошибается тот, кто ничего не делает.", author: "Теодор Рузвельт" },
  { content: "То, что мы знаем, — это капля, а то, чего мы не знаем, — это океан.", author: "Исаак Ньютон" },
  { content: "Сложнее всего начать действовать, все остальное зависит только от упорства.", author: "Амелия Эрхарт" },
  { content: "Успех — это способность шагать от одной неудачи к другой, не теряя энтузиазма.", author: "Уинстон Черчилль" },
  { content: "Логика может привести вас от пункта А к пункту Б, а воображение — куда угодно.", author: "Альберт Эйнштейн" }
];
...
async function renderRandomQuote() {
  ...
  // Функция fetch с жестким таймаутом (чтобы не ждать повисшие API)
  ...
  try {
    // Используем надежный API-прокси
    const url = 'https://services.bonjourr.fr/quotes/classic/ru';
    const response = await fetchWithTimeout(url, 2500); // Таймаут 2.5 секунды
    ...
    if (response.ok) {
      ...
      if (data && data.length > 0) {
        finalQuote = {
          content: data[0].content || data[0].text,
          author: data[0].author || "Неизвестный автор"
        };
      }
    }
  } catch (err) {
    console.warn('[NovaTab] API цитат недоступно, используется локальный резерв.');
  }

  // Применяем текст, обрабатывая переносы строк
  quoteEl.textContent = `«${(finalQuote.content || '').replace(/\\n/g, '\n')}»`;

...
> **Статус: РЕАЛИЗОВАН V1 (2026-06-14).** Backend Project Memory + Telegram Capture
> построен; миграции применены, проверено живьём (см. `STATE_CURRENT.md` §4/§10,
> `handoff.md`). **Важное отклонение от этого ADR:** `project_items` НЕ создавали —
> консолидировали в единый `memory_items` (универсальный контейнер знаний) по
> решению пользователя и принципу «не создавать параллельные системы памяти».
> Поэтому ниже секции про `project_items` — исторический замысел; в коде их роль
> играет `memory_items` (+ `memory_links` для связей, AI-теггер вместо ручных
> kind/review). Не реализовано в V1: UI `/projects`, голос в Telegram, векторные
> эмбеддинги items, обход графа в recall.

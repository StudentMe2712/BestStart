...
function Editor({
  ...
  const init = useMemo(
    () => (item ? parseSolution(item.content) : null),
    [item]
  )
  const [title, setTitle] = useState(item?.title || "")
  const [status, setStatus] = useState<SolutionStatus>(
    item ? statusOf(item.tags) : "unresolved"
  )
  const [category, setCategory] = useState(item ? catOf(item.tags) : "other")
  const [tagsStr, setTagsStr] = useState(
    item ? freeTags(item.tags).join(", ") : ""
  )
  const [projectId, setProjectId] = useState(item?.project_id || "")
  const [f, setF] = useState<SolutionFields>(
    init || { problem: "", cause: "", solution: "", commands: "", notes: "" }
  )
  ...
  async function save() {
    ...
    const free = tagsStr.split(",").map((t) => t.trim()).filter(Boolean)
    const tags = buildTags(category, status, free)
    setSaving(true)
    onError("")
    try {
      const body = {
      ...
      <div
        ...
        <div className="flex items-center justify-between px-5 h-14 border-b border-neutral-800">
          <h3 className="text-base font-semibold">
            {item ? "Редактировать решение" : "Новое решение"}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4 font-sans">
          ...
          <Field label="Теги (через запятую)">
            <input
              ...
              placeholder="pgvector, embedding"
              className="w-full bg-neutral-900 border border-neutral-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-lime-400/50"
            />
          </Field>
          ...
          <Area label="Решение" value={f.solution} onChange={(v) => set("solution", v)} />
          <Area
            label="Команды"
            value={f.commands}
            onChange={(v) => set("commands", v)}
            mono
          />

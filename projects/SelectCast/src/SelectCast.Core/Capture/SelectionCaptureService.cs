...
/// Must be called on the STA dispatcher thread (Clipboard.* requires STA); the awaits
/// resume there too via <c>ConfigureAwait(true)</c>.
/// </summary>
public sealed class SelectionCaptureService
{
    public async Task<CaptureResult> CaptureAsync(int timeoutMs = 700, int pollIntervalMs = 15)
    {
        Debug.Assert(
            ...
            "CaptureAsync must run on an STA thread (Clipboard requires STA).");
        ...
        try
        {
            ...
            uint sent = SendCopyKeystroke(diag);
            if (sent == 0)
            {
                diag.AppendLine("BLOCKED: SendInput delivered 0 events (UIPI / input blocking)");
                return new CaptureResult(CaptureStatus.Blocked, null, 0, diag.ToString());
            }
        ...
        {
            // Clipboard is sacred — restore no matter what happened above (covers every early return).
            try
            {
    ...
    /// <returns>Number of key events SendInput actually injected (0 ⇒ blocked).</returns>
    private static uint SendCopyKeystroke(StringBuilder diag)
    {

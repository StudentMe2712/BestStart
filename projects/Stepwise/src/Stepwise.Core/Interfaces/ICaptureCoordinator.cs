...
public interface ICaptureCoordinator
{
    ...
    /// <summary>
    /// Асинхронно выполняет захват экрана с получением подробных метаданных результата захвата (размеры, координаты подсветки).
    /// </summary>
    /// <param name="sequenceIndex">Порядковый номер шага.</param>
    /// <param name="target">Целевой элемент пользовательского интерфейса.</param>
    /// <param name="cancellationToken">Токен отмены операции.</param>
    /// <returns>Результат операции захвата <see cref="CaptureResult"/>.</returns>
    Task<CaptureResult> CaptureStepWithResultAsync(int sequenceIndex, ElementInfo target, CancellationToken cancellationToken = default);
}
